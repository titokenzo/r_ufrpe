...
# v<-c(20,30,4,934,1077,22), submeta a função paste() que apenas adiciona a 
# unidade metro (‘m’) após cada número do vetor. Note que não pode haver espaço
# entre o número e a unidade de medida. Além disso, use aspas simples e remova 
# todos os espaços em banco do comando.
v<-c(20,30,4,934,1077,22)
v1 <- paste(v,'m',sep='')

# Copie o poema "No Meio do Caminho" de Drummond através do 
# link: https://www.culturagenial.com/poema-no-meio-do-caminho-de-carlos-drummond-de-andrade/. 
# Note que tem que ser copiado e colado exatamente como está no site.
# Calcule e submeta a quantidade de caracteres totais, incluindo os espaços em
# branco e as quebras de linha(‘\n’).
...
# Dado a string txt = "eeer3GHJca1nfgSigmanfaUIOntafan", qual a expressão 
# regular que deve, para que ela extraia apenas números e letras maiúsculas. 
# Note que você precisa ser adicionada no atributo ‘pattern’ da função 
# str_extract_all(string=txt,pattern=" ") carregar o pacote stringr. 
# Também use o Regex.
library(stringr)
...
str_extract_all(string=txt,pattern="UFRPE[a-z0-9]+PPGIA")

# Copie o discurso de Marthin Luther King Jr. do seguinte 
# link: http://www.dhnet.org.br/desejos/sonhos/dream.htm . 
# Qual a expressão regular trás "negro", "negros", "Negro", "Negros", "negra" 
# e "negras"? 
...
str_extract_all(string=txt,pattern="(n|N)egr(o|a)s?")

# Qual das opções abaixo correspondem a regexp "a(ab)*a" ?
txt = c("abababa","aaba","aabbaa","aba","aabababa")
grep(pattern = "a(ab)*a", txt,value = T) #"aaba"     "aabbaa"   "aabababa"

# Qual das opções abaixo correspondem a regexp "ab+c?" ?
txt = c('abc','ac','abbb','bbc')
grep(pattern = "ab+c?", txt,value = T) #"abc"  "abbb"

# Qual das opções abaixo correspondem a regexp "a.[bc]+" ?
txt = c('abc','abbbbbbbb','azc','abcbcbcbc','ac','asccbbbbcbcccc')
grep(pattern = "a.[bc]+", txt,value = T) #"abc" "abbbbbbbb" "azc" "abcbcbcbc" "asccbbbbcbcccc"

# Qual das opções abaixo correspondem a regexp "abc|xyz" ?
txt = c('abc','xyz','abc|xyz')
grep(pattern = "abc|xyz", txt,value = T)

# Qual das opções abaixo correspondem a regexp "[a-z]+[\.\?!]" ? 
txt = c('battle!','Hot','green','swamping.','jump up.','undulate?','is.?')
grep(pattern = "[a-z]+[\\.\\?!]", txt,value = T)
...
# Usando quantificadores, encontre todos os países com ee, mas não com eee, 
# em seu nome. Abaixo, digite a expressão regular.
teste = c("Teea","Teeeeea","eer","err","eeer")
...
# (i) começam com “Ba”; 
# (ii) terminam com “land”; e 
# (iii) tenham uma palavra no nome que começa com “Ga” 
# (dica: veja \b). 
# Após isso, submeta o total de países encontrados para os itens anteriores.
paises_i <- grep(pattern = "^Ba", aux$country, value = T)
paises_ii <- grep(pattern = "land$", aux$country, value = T)
paises_iii <- grep(pattern = "Ga", aux$country, value = T)

str_extract_all(aux$country, pattern = "^Ba.*land$")
str_view(aux$country, pattern = "^Ba")
saida_i <- data.frame( unique(paises_i))
saida_ii <- data.frame( unique(paises_ii))
saida_iii <- data.frame( unique(paises_iii))
