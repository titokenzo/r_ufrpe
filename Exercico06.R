...
# Quais comandos no R retornam um vetor das médias das variáveis 'Sepal.Length',
# 'Sepal.Width', 'Petal.Length' e 'Petal.Width' do dataset iris 
# (já incluído no R) ?
...
# Quais comandos calculam a média do tamanho das pétalas (Petal.Length) de 
...
# entre 'Rural' e 'Amo'. O comprimento desses 10 elementos diminuem de 10 para 
# 1 (ver abaixo a saída). 
...
# impressas: 1:10, 2:11 e 3:12 (ver abaixo a saída)? 
# OBS.: Ao submeter o código, retire todos os espaço em branco e não use 
# parênteses.
...
# sexo= c ("f", "f", "m", "m"), anos= c (21,15,17,19)). 
...
# Caso contrário, atribua "F" (veja o resultado esperado abaixo). Digite apenas
...
str(student.df)
?ifelse
student.df <- transform(student.df,menor=ifelse(sexo=="m" & anos<18,"V","F"))
student.df$menor <- with(student.df, ifelse(sexo=="m" & anos<18, "V","F"))
student.df$menor<-ifelse(student.df[,"sexo"]=="m"&student.df[,"anos"]<18,"V","F")
student.df

# Qual o comando, usando a família apply, você consegue obter a soma de cada um 
# dos crimes contido no dataset USArrests (já incluído no R) com a saída igual 
